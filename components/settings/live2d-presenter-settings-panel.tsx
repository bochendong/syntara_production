...
export function Live2dPresenterSettingsPanel({ className }: { className?: string }) {
  ...
  const [draftModel, setDraftModel] = useState<typeof live2dPresenterModelId | null>(null);
  const [draftVisible, setDraftVisible] = useState<boolean | null>(null);
  const selectedModel = draftModel ?? live2dPresenterModelId;
  const selectedVisible = draftVisible ?? live2dPresenterVisible;
  const dirty =
    selectedModel !== live2dPresenterModelId || selectedVisible !== live2dPresenterVisible;
  return (
    <div className={cn('space-y-3', className)}>
      <div>
        <Label className="text-sm font-medium">伴学角色</Label>
        <p className="mt-1 text-xs text-muted-foreground">
          选择学习主页和课程中的伴学角色，点击保存后生效。
        </p>
      </div>
      ...
      <Button
        disabled={!dirty}
        onClick={() => {
          setLive2DPresenterModelId(selectedModel);
          setLive2DPresenterVisible(selectedVisible);
          setDraftModel(null);
          setDraftVisible(null);
        }}
      >
        {dirty ? '保存伴学角色' : '已保存'}
      </Button>
