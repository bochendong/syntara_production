...
export default function CourseDetailPageClient() {
  ...
  if (!authHydrated || !isLoggedIn || portalRole !== 'STUDENT') return null;

  if (loading && !dashboard) {
    return <CourseWorkspaceLoadingContent />;
  }

  if (!dashboard) {
    return (
      <main className="grid min-h-full place-items-center bg-[#f3f6fb] p-6 dark:bg-[#0e1117]">
    ...
    <main className="min-h-full bg-[#f3f6fb] text-slate-950 dark:bg-[#0e1117] dark:text-white">
      <div className="mx-auto flex w-full max-w-[86rem] flex-col gap-4 px-3 py-4 md:px-5 lg:px-6">
        <CourseSpaceHeader
          ...
          courseAvatarUrl={avatarUrl}
          role="student"
          active="dashboard"
          problemCount={metrics.total}
        />
