...
type NotebookProblem = Awaited<ReturnType<typeof listNotebookProblemsForUser>>[number];

function problemsForImportBatch(problems: NotebookProblem[], importBatchId: string) {
...
async function readBatchProblemState(userId: string, notebookId: string, importBatchId: string) {
  const problems = await listNotebookProblemsForUser(userId, notebookId);
  return {
    problems,
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  return safeRoute(async () => {
    ...
    let problems: Awaited<ReturnType<typeof createNotebookProblemsFromDrafts>>;
    ...
    try {
      ...
      if (importBatchId && importBatch) {
        const state = await readBatchProblemState(auth.userId, id, importBatchId).catch(() => ({
          problems: [] as NotebookProblem[],
          batchProblems: [] as NotebookProblem[],
        }));
    ...
    scheduleUnlinkedCourseKnowledgeProjectionSync({
      ...
      ownerId: auth.userId,
      reason: 'notebook_problems_imported',
    });
    ...
    return NextResponse.json({
      ...
      import: writeSummary
        ? {
            ...
            persistedInsertedCount: writeSummary.insertedProblemIds.length,
