...
- Avoid lesson-plan language in `title`, `description`, `keyPoints`, `teachingObjective`, and `studentThinkingMove`: do not use phrases such as "let students see", "students should understand", "this page is used to", "teaching objective", "lesson spine", "lecture focus", "transferable action", or "establish the main thread".
