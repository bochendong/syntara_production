...
6. 课程资料/教师资料接口验证文件凭证，从 OpenAI 下载一次原文件做确定性解析，同时持久化原文件和 `file_id`。
7. 聊天请求只携带短期 `file_id`。检测到文件输入时，后端强制使用 OpenAI Responses API，并构造 `input_file`；回答结束后删除临时聊天文件。
...
- PPTX/DOCX 直接文件输入主要提供文本；若以后需要完整读取图表和版式，应在后台生成一个派生 PDF，并同时保留原文件。
