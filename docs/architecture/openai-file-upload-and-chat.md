...
6. 教师资料由浏览器分片保存进业务数据库，上传完成不会自动启动 AI；老师手动点击“生成笔记本”或“导入题库”后才创建 AI 队列任务。题库任务在处理阶段把数据库中的权威原文件上传至 OpenAI，并持久化 `file_id` 供重试复用；OpenAI 或文件解析失败不会回滚已经保存的原文件。
...
题库文件属于课程，不属于笔记本。课程题库页不再提供独立的“预览后导入”链路；老师统一从课程资料库上传题库原文件，再手动触发“导入题库”。题库资料处理强制 `notebookId = null`，只创建课程题目，不创建 AI 笔记本；以后如需归类，由老师在独立的整理流程中显式操作。
